...
from robotdataprocess import OdometryData, CoordinateFrame, CameraData, TransformationData, TransformType
...
def publish_data(input_dir: str, robot_name: str):
    ...
    # Extract Robot to Camera transformation
    ...
    H_R_to_O_in_FLU = H_R_to_O.to_coordinate_frame(CoordinateFrame.FLU, TransformType.ROTATION)
    H_R_to_O_in_FLU.frame_id = "body"
    H_R_to_O_in_FLU.child_frame_id = 'camera_color_optical_frame'

    # Define Camera parameters
    left_camera_data = CameraData.from_user_mono('/cam0', 752, 480, 376, 376, 376, 240, CameraData.DistortionModel.RADIAL_TANGENTIAL)
    left_camera_data.sync_to_ImageData(left_image_data)

    # Multiply all timestamps by 5 to simulate traveling 5 times slower
    for data in [left_image_data, depth_data, left_camera_data, odom_data]:
        data.timestamps = data.timestamps * Decimal('5.0')
    odom_data.interpolate_to_hz(4)

    # Publish the data via ROS topics
    publish_data_ROS_multiprocess([left_image_data, depth_data, left_camera_data, left_camera_data, odom_data, H_R_to_O_in_FLU], 
                                  ['/robot/cam0/image_raw',
                                   '/robot/cam0/depth/image_raw',
                                   '/robot/cam0/camera_info',
                                   '/robot/cam0/depth/camera_info',
                                   '/tf', 
                                   '/tf_static'],
                                    [None, None, None, None, "TFMessage", None], 
                                    [20, 20, 20, 20, 20, 1],
                                    [2, 2, 1, 1, 1, 1], ROSMsgLibType.ROSPY, True, verbose=True, publish_clock=False)
